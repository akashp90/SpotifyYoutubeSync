...
def spotify_search_for_songs(video_titles):
    ...
    for video_title in video_titles:
        ...
        params = {"q": video_title, "type": "track"}

        response = requests.get(
            SPOTIFY_BASE_URI + "/v1/search", headers=headers, params=params
        )
        response_body = json.loads(response.content)

        # print("Response: " + str(response_body))
        spotify_tracks[video_title] = convert_spotify_search_response_to_track_object(
            response_body
        )
...
def convert_spotify_search_response_to_track_object(response_body):
    ...
    artist_names = [
        artist["name"]
        for artist in response_body.get("tracks").get("items")[0]["artists"]
    ]
    # Combine the names into a single string
    combined_names = ", ".join(artist_names)
    result = {
        "name": response_body.get("tracks").get("items")[0]["name"],
        "spotify_url": response_body.get("tracks").get("items")[0]["external_urls"][
            "spotify"
        ],
...
def youtube_playlists():
    ...
    request = youtube.playlists().list(
        part="contentDetails,id,snippet,status", mine=True
    )
    response = request.execute()
...
def check_enable_sync_button():
    res = False
    if session.get("youtube_source_playlist_id") and session.get(
        "spotify_destination_playlist_id"
    ):
        return True
    if session.get("spotify_source_playlist_id") and session.get(
...
def spotify_after_login():
    ...
    response = requests.post(
        SPOTIFY_ACCOUNTS_BASE_URL + "/api/token", data=payload, headers=headers
    )
    response_body = json.loads(response.content)
...
def add_tracks_to_spotify_playlist(track_objects, playlist_id):
    ADD_TRACK_TO_PLAYLIST_URL = (
        SPOTIFY_BASE_URI + "/v1/playlists/" + playlist_id + "/tracks"
    )
    ...
    for uris in divide_chunks(all_uris, 100):
        ...
        body = {"uris": uris}
        response = requests.post(
            ADD_TRACK_TO_PLAYLIST_URL, headers=headers, data=json.dumps(body)
        )
